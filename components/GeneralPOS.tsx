...
const STATIC_SALES: GeneralSale[] = [
  {
    id: 'gs-1',
    receiptNumber: 'GS-000001',
    items: [
      { productId: 'prod-1', productName: 'Rice 5kg', price: 7500, quantity: 2, total: 15000 }
    ],
    totalQuantity: 2,
    subtotal: 15000,
    discount: 0,
...
export default function GeneralPOS() {
  ...
  const totals = {
    subtotal: cart.reduce((sum, item) => sum + (item.product.sellingPrice * item.quantity), 0),
    totalQuantity: cart.reduce((sum, item) => sum + item.quantity, 0),
    grandTotal: Math.max(0, cart.reduce((sum, item) => sum + (item.product.sellingPrice * item.quantity), 0) - discountInput),
    balance: Math.max(0, cashReceivedInput - Math.max(0, cart.reduce((sum, item) => sum + (item.product.sellingPrice * item.quantity), 0) - discountInput))
  };
  ...
  const handleCheckout = (e: React.FormEvent) => {
    ...
    const newSaleID = `gs_${sales.length + 1}`;
    ...
    setProducts(prev => prev.map(item => {
      const cartItem = cart.find(cartEntry => cartEntry.product.id === item.id);
      if (!cartItem || item.category === 'Services') return item;
      return {
        ...item,
      ...
      <div className="flex border-b border-slate-100 dark:border-slate-700/50 justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-xl">
        <div className="flex gap-1">
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-lg transition flex items-center gap-1.5 ${activeTab === 'checkout'
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-lg transition flex items-center gap-1.5 ${activeTab === 'history'
                ? 'bg-indigo-600 text-white'
                : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700/50'
              }`}
          >
          ...
          <div className="lg:col-span-7 space-y-4" id="pos-lhs-catalog">
            ...
            <div className="bg-white dark:bg-gray-800 p-4 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <form onSubmit={handleBarcodeSubmit} className="hidden relative">
              ...
              <div className="flex gap-1 overflow-x-auto pb-1 scrollbar-thin">
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold rounded-lg shrink-0 transition ${selectedCategory === cat
            ...
            {/* Catalog Grid */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 overflow-y-auto max-h-145 p-0.5" id="products-catalog-grid">
                ...
                visibleProducts.map(product => {
                  ...
                  return (
                    <div
                      ...
                      className={`group p-4 bg-white dark:bg-gray-800 rounded-2xl border transition h-48 flex flex-col justify-between cursor-pointer relative shadow-xs overflow-hidden ${isOutOfStock
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg shadow-xl overflow-hidden border border-slate-100 dark:border-slate-700">
            ...
            <div className="p-4 border-b bg-slate-50 dark:bg-slate-700/50 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <span className="text-xs uppercase font-bold text-slate-600">Print Specifications:</span>
                <div className="inline-flex rounded-md border border-slate-200 dark:border-slate-600 overflow-hidden bg-white dark:bg-slate-700 text-[10px] font-bold">
                  ...
                  <button
                    ...
                    className={`hidden px-2.5 py-1 ${printFormat === 'a4' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
                  >
                    A4 Page Format
                  </button>
                </div>
            ...
            {/* Layout Wrapper */}
            <div className="p-6 max-h-137.5 overflow-y-auto bg-slate-100 dark:bg-slate-900/40 flex justify-center">
              ...
              {printFormat === 'thermal' ? (
                ...
                >
                  ...
                  <div className="text-center space-y-1">
                    <h2 className="text-sm font-black uppercase">TITAN GENERAL SALES</h2>
                    <p className="text-[8px] text-slate-500">Cooking Gas Plant & General Store</p>
                    <p className="text-[8px] max-w-50 mx-auto text-slate-500">Plot 12, Lekki-Epe Expressway, Lagos State</p>
                    ...
                    <p className="font-bold text-xs">PROVISION SALES RECEIPT</p>
                  </div>
